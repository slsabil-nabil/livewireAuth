<div>
    <x-page-title title="عرض الصلاحيات" subtitle="عرض الصلاحيات المتاحة في النظام" align="right">
        <div
            class="text-[rgb(var(--primary-600))] bg-[rgba(var(--primary-100),0.25)] border border-theme px-4 py-2 rounded-lg flex items-center gap-2">
        ...
        @if ($permissions->count() > 0)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                @foreach ($grouped as $module => $perms)
                    <div
                        class="bg-white/80 border border-theme rounded-2xl shadow-xl p-4 flex flex-col transition hover:shadow-2xl hover:scale-[1.02] duration-200">
                        <h3 class="text-lg font-bold mb-4 flex items-center gap-2 text-[rgb(var(--primary-600))]">
                            <i
                                class="fas {{ $iconMap[$module] ?? $iconMap['default'] }} text-[rgb(var(--primary-500))] text-xl"></i>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y rounded-xl overflow-hidden divide-gray-200">
                                <thead style="background:rgba(var(--primary-100),0.5)">
                                    <tr>
                                        <th
                                            class="px-4 py-2 text-right text-xs font-medium text-[rgb(var(--primary-600))] uppercase tracking-wider">
                                            اسم الصلاحية</th>
                                        <th
                                            class="px-4 py-2 text-right text-xs font-medium text-[rgb(var(--primary-600))] uppercase tracking-wider">
                                            الإجراء</th>
                                    ...
                                    @foreach ($perms as $permission)
                                        <tr>
                                            <td class="px-4 py-2 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    ...
                                                    @if (in_array($permission->name, $basicPermissions))
                                                        <span
                                                            class="mr-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                                            أساسية
                                                        </span>
                                                    @endif
                                                </div>
                                            ...
                                            <td class="px-4 py-2 whitespace-nowrap">
                                                ...
                                                @endphp
                                                <span
                                                    class="text-xs font-semibold text-[rgb(var(--primary-500))]">{{ ucfirst($action) }}</span>
                                            </td>
