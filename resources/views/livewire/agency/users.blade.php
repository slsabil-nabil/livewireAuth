<div class="space-y-6">
    <!-- العنوان الرئيسي -->
    <x-page-title title="إدارة المستخدمين" align="right">
        @can('users.create')
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-2 py-1">
                            <div class="flex gap-2">
                                @can('users.edit')
                                    <button wire:click="editUser({{ $user->id }})" class="text-xs font-medium"
            ...
            <div
                ...
                <form wire:submit.prevent="{{ $showEditModal ? 'updateUser' : 'addUser' }}" class="space-y-4 text-sm">
                    ...
                    <div class="{{ $containerClass }}">
                        <input type="text" wire:model.defer="{{ $showEditModal ? 'edit_name' : 'name' }}"
                            class="{{ $fieldClass }}" placeholder="الاسم" />
                        <label class="{{ $labelClass }}">الاسم</label>
                        @error($showEditModal ? 'edit_name' : 'name')
                    ...
                    <!-- البريد -->
                    <div class="{{ $containerClass }}">
                        <input type="email" wire:model.defer="{{ $showEditModal ? 'edit_email' : 'email' }}"
                            class="{{ $fieldClass }}" placeholder="البريد الإلكتروني" />
                        <label class="{{ $labelClass }}">البريد الإلكتروني</label>
                        @error($showEditModal ? 'edit_email' : 'email')
                            <span class="text-red-600 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="{{ $containerClass }}">
                        ...
                        <label
                            class="{{ $labelClass }}">{{ $showEditModal ? 'كلمة مرور جديدة' : 'كلمة المرور' }}</label>
                        @error($showEditModal ? 'edit_password' : 'password')
                            <span class="text-red-600 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <!-- الدور -->
                    <div class="{{ $containerClass }}">
                        <select wire:model.defer="{{ $showEditModal ? 'edit_role' : 'role' }}"
                            class="{{ $fieldClass }}">
                        ...
                        @error($showEditModal ? 'edit_role' : 'role')
                            <span class="text-red-600 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
