...
<div class="space-y-6">
    <!-- العنوان والرسائل -->
    <x-page-title title="إدارة المزودين">
        @if (session('message'))
    ...
    <div class="bg-white rounded-xl shadow-md p-4">
        <div class="flex justify-between items-center mb-4">
            <x-section-title title="قائمة المزودين" />
                    ...
                    @forelse($providers as $provider)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-2 py-1">
                                @if ($provider->type)
                                    <span
                                        class="inline-flex px-1.5 py-0.5 text-2xs font-semibold rounded-full bg-blue-100 text-blue-800">
                                        {{ $provider->type }}
                                    </span>
                                @else
                                    -
    ...
    @if ($showModal)
        <div class="fixed inset-0 z-50 bg-black/10 flex items-center justify-center backdrop-blur-sm">
            <div
                class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 p-6 relative transform transition-all duration-300">
                ...
                <form wire:submit.prevent="saveProvider" class="space-y-4 text-sm">
                    ...
                    <!-- نوع المزود -->
                    <div class="{{ $containerClass }}">
                        <input type="text" wire:model.defer="type" class="{{ $fieldClass }}"
