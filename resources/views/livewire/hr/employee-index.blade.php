<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <x-page-title title="قائمة الموظفين">
            إدارة معلومات موظفي الوكالة.
        </x-page-title>
            ...
            <div class="relative mt-1">
                <select wire:model.lazy="department_id"
                    ...
                    @foreach ($departments as $id => $name)
                        <option wire:key="dep-{{ $id }}" value="{{ $id }}">{{ $name }}
                        </option>
                    @endforeach
                </select>
                <label
            ...
            <div class="relative mt-1">
                <select wire:model.lazy="position_id"
                    ...
                    @foreach ($positions as $id => $name)
                        <option wire:key="pos-{{ $id }}" value="{{ $id }}">{{ $name }}
                        </option>
                    @endforeach
                </select>
                <label
            ...
            <div
                ...
                <form wire:submit.prevent="{{ $editingEmployee ? 'updateEmployee' : 'addEmployee' }}"
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <!-- الفرع -->
                        <div class="{{ $containerClass }}">
                            <input wire:model="branch" type="text" class="{{ $fieldClass }}"
                                placeholder="الفرع" />
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden"
        wire:key="employees-table-{{ $employees->count() }}-{{ now() }}">
